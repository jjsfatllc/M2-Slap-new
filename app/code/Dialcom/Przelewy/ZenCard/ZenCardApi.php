...
//    const API_BASE_URL = 'https://sandbox.przelewy24.pl/api/v1/zencard/';
    ...
    public function __construct($merchantId, $apiKey)
    {
        ...
        $this->userAgent = Util::getRemoteUserAgent();
    }
    ...
    public function getScript()
    {
        ...
        $this->call();
        $decodedResponse = json_decode($this->response);
        $url = isset($decodedResponse->url) ? $decodedResponse->url : '';
    ...
    private function getTransactionData($email, $amount, $zenCardOrderId)
    {
        ...
        $cookies = array(
            'zcodeCookie' => Util::getCookie(Util::ZCODE_COOKIE, ''),
            'extraInfoCookie' => Util::getCookie(Util::EXTRA_INFO_COOKIE, ''),
            'discountCookie' => Util::getCookie(Util::DISCOUNT_COOKIE, '')
        );
        ...
        $data = array(
            ...
            'p24_ip' => Util::getRemoteIp(),
            'p24_order_id' => $zenCardOrderId,
            'p24_amount_discount' => Util::getAmountWithDiscount($amount)
        );

        $this->post = http_build_query($data);
        $this->call();
        $decodedResponse = json_decode($this->response);
        $decodedTransaction = isset($decodedResponse->transaction) ? $decodedResponse->transaction : null;
        $transaction = json_decode($decodedTransaction);
